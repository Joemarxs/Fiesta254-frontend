...
import { eventCategories, mockEvents } from '../data/mockData';
...
const EditEventPage = () => {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || !currentUser?.isHost) {
      navigate('/login');
    }
    if (event) {
      ...
      // Initialize ticket options based on the event price
      setTicketOptions([{
        id: '1',
        name: 'Standard',
        price: event.price.toString(),
        availableFrom: event.date,
        availableTo: event.date,
        description: 'Regular admission ticket'
      }]);
    }
  }, [event, isAuthenticated, currentUser, navigate]);
  if (!event) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For this mock app, we'll just navigate back to the dashboard
    // Create a notification or success message here
    navigate('/dashboard');
  };
  return <div className="bg-gray-50 min-h-screen py-8">
      <div className="container mx-auto px-4">
        <Link to="/dashboard" className="flex items-center text-indigo-600 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                ...
                <input type="text" id="title" value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
              </div>
              {/* Event Description */}
              <div className="mb-6">
                ...
                <textarea id="description" rows={6} value={description} onChange={e => setDescription(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required></textarea>
              </div>
              {/* Category */}
              <div className="mb-6">
              ...
              <div className="mb-6">
                ...
                <input type="text" id="location" value={location} onChange={e => setLocation(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
              </div>
              {/* Ticket Options */}
              <div className="mb-8">
              ...
              <div className="mb-6">
                ...
                <input type="number" id="capacity" value={capacity} onChange={e => setCapacity(e.target.value)} min="1" className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
              </div>
              {/* Image Upload */}
              <div className="mb-6">
              ...
              <div className="flex justify-end space-x-4">
                <Link to="/dashboard" className="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition duration-300">
                  Cancel
                </Link>
